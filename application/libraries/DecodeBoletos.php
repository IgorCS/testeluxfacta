...
class DecodeBoletos{
    ...
    public function getCodVerificador(){
        ...
        if (empty($this->codVerificador)){
            if(!empty($this ->barras)){
                $this->calcVerificadorBarras($this->barras);
                $codVerificadorBarra = $this ->codVerificador = substr($this ->barras, 4, 1);
               // echo "Verifica Barra:".$codVerificadorBarra;

            }elseif(!empty($this ->ipte)){
                $codVerificadorIpte = $this ->codVerificador = substr($this ->ipte, 38, 1);
                // var_dump("Verifica IPTE:".$codVerificadorIpte);
                //exit();
            }else{
    ...
    for($i = 0; $i < sizeof($fatores); $i++){
         ...
         $soma += (int)$barras[$i] * $fatores[$i];
         //var_dump($soma);
        }
        $mod = $soma % 11;
        $resultado = 11 - $mod;
       
        $verificaCod = $this ->codVerificador = $resultado;
       // print_r("VERIFICADOR:".$verificaCod."<br />");
